...
class KuramotoSystem:
    ...
    def initialize(self, sigma=1.0, omega=None, theta_0=None, A=None, module_id=None):
        if omega is None:
            # self.omega  = sigma * np.random.standard_cauchy(self.N) # Distribución lorentziana
            self.omega  = np.random.normal(0.0, sigma, self.N) # Distribución Gaussiana
        else:
...
def R_modelo(K, Kc, R_inf):
    """Modelo Kuramoto exacto para distribucion Lorentziana g(omega).

        R(K) = R_inf * sqrt( max(0, 1 - Kc/K) )

    Esta es la solucion analitica del Kuramoto de campo medio cuando
    g(omega) es Lorentziana de anchura gamma: Kc = 2*gamma exacto, sin
    parametros de forma libres. R_inf permite saturacion < 1 por
    finite-size.
    """
...
def ajustar_R_simple(K_data, R_mean, R_err, Kc_guess,
                     ...
                     R_inf_max=1.05):
    """Ajusta R_modelo (Lorentziana exacta) por minimos cuadrados pesados.
    Devuelve (popt, perr) con popt = [Kc, R_inf]."""
    ...
    p0     = [Kc_guess, 1.0]
    bounds = ([max(K_fit.min() * 0.1, 1e-6), 0.3],
              [K_fit.max() * 3.0,             R_inf_max])

    # Clip p0 a bounds (evita fallos cuando Kc_guess viene de un pico
    # espurio fuera del rango fisico filtrado).
    p0 = [np.clip(p0[k], bounds[0][k], bounds[1][k]) for k in range(2)]
...
def ajustar_todas_las_curvas(K_values, R_means, R_mean_stds, R_stds,
    ...
    # --- Resumen por consola ---
    # --- Resumen por consola ---
    print("\n  Ajustes R_modelo Lorentziano (Kc, R_inf) +/- 1-sigma:")
    print(f"  {'curva':>8} | {'Kc':>14} | {'R_inf':>12} | nota")
    for m in range(num_modules):
        p, e = fits[f'm{m}']['popt'], fits[f'm{m}']['perr']
        print(f"  {'r_'+str(m+1):>8} | {p[0]:>7.4g} +/-{e[0]:>5.2g} "
              f"| {p[1]:>5.3f} +/-{e[1]:>5.2g} |")
    p, e = fits['global']['popt'], fits['global']['perr']
    nota = (f"K > {K_min_fit_glob:.3g}" if K_min_fit_glob else "rango completo")
    print(f"  {'R glob':>8} | {p[0]:>7.4g} +/-{e[0]:>5.2g} "
          f"| {p[1]:>5.3f} +/-{e[1]:>5.2g} | {nota}")
...
def parametros_simulacion():
    ...
    num_K        = 10
    K_min        = 5e-3
    K_max        = 0.95 * (2 / (dt * (N//num_modules)))           # Aquí estamos limitando las K, para que no nos salgamos del rango estable del modelo.
    K_center     = None
    width_factor = 0.5
    ...
    method_temp  = 'euler'            # integrador para el modo TEMP
